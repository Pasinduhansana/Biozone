...
const Map = () => {
    // State to control the visibility of the modal and preloading
    const [selectedTimetable, setSelectedTimetable] = useState(null);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [isIframeLoaded, setIsIframeLoaded] = useState(false);

    // Function to open the modal and preload content
    ...
    return (
        <div className="container mx-auto relative">
                ...
                <div className="flex flex-wrap gap-6 w-3/5 justify-center">
                    {data.map((institute, index) => (
                        <div
                            ...
                            style={{
                                backgroundImage: 'url(https://images.unsplash.com/photo-1541829070764-84a7d30dd3f3?q=80&w=2069&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D)',
            ...
            {isModalOpen && selectedTimetable && (
                  <motion.div
                  className="fixed inset-0 flex items-center justify-center backdrop-blur-[10px] bg-opacity-10 z-50"
                  initial={{ opacity: 0, scale: 0.8 }}
                  animate={{ opacity: 1, scale: 1 }}
                  exit={{ opacity: 0, scale: 0.8 }}
                  transition={{ duration: 0.3, ease: "easeInOut" }}
              >
                <div className="fixed inset-0 flex items-center justify-center backdrop-blur-[10px] bg-opacity-10 z-50">
                    <div className="bg-white lg:w-4/5 lg:h-auto w-full h-full relative rounded-lg">
                        <button
                            className="absolute right-1 m-3 p-1 bg-white border border-solid border-gray-300 px-2 py-2 rounded-full hover:scale-105"
                            onClick={closeModal}
                        >
                            <VscChromeClose />
                        </button>
                        ...
                        <iframe
                            ...
                            src={selectedTimetable.addressLink}
                            width="600"
                            height="600"
                            title="Map"
                            loading="lazy"
                            onLoad={() => setIsIframeLoaded(true)} // Track iframe load state
                        />
